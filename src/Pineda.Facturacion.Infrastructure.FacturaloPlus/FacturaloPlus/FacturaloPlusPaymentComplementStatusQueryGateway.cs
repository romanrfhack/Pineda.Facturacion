...
public class FacturaloPlusPaymentComplementStatusQueryGateway : IPaymentComplementStatusQueryGateway
{
    ...
    public FacturaloPlusPaymentComplementStatusQueryGateway(
    ...
    public async Task<PaymentComplementStatusQueryGatewayResult> QueryStatusAsync(PaymentComplementStatusQueryRequest request, CancellationToken cancellationToken = default)
    {
        ...
        var payload = new PaymentComplementStatusPayload
        {
            Uuid = request.Uuid,
        ...
        using var httpRequest = new HttpRequestMessage(HttpMethod.Post, _options.PaymentComplementStatusQueryPath)
        {
        ...
        {
            return new PaymentComplementStatusQueryGatewayResult
            {
    ...
    private static string BuildRawResponseSummary(HttpStatusCode statusCode, PaymentComplementStatusResponse? response, string rawContent)
    {
        var summary = new
        {
    ...
    private static PaymentComplementStatusResponse? TryDeserialize(string rawContent)
    {
        ...
        try
        {
            return JsonSerializer.Deserialize<PaymentComplementStatusResponse>(rawContent, JsonOptions);
        }
        catch (JsonException)
        {
            return null;
    ...
    private PaymentComplementStatusQueryGatewayResult ValidationFailed(string errorMessage)
    {
        return new PaymentComplementStatusQueryGatewayResult
        {
            Outcome = PaymentComplementStatusQueryGatewayOutcome.ValidationFailed,
            ProviderName = _options.ProviderName,
            ProviderOperation = "payment-complement-status-query",
            CheckedAtUtc = DateTime.UtcNow,
    ...
    private PaymentComplementStatusQueryGatewayResult Unavailable(string errorMessage)
    {
        return new PaymentComplementStatusQueryGatewayResult
        {
            Outcome = PaymentComplementStatusQueryGatewayOutcome.Unavailable,
    ...
    private sealed class PaymentComplementStatusPayload
    {
    ...
    private sealed class PaymentComplementStatusResponse
    {
